...
`frame 번호`를 `저장할 자료형`을 int형이라 하자. int형은 4Bytes = 32Bits니까 2^32개의 숫자를 표현할 수 있다.

앞서 프레임 하나의 크기는 `4KB = 4 × 2^10bytes`가 되는데... 숫자 하나가 프레임 하나를 표현할 수 있다.

그런데, frame 번호로 쓸 수 있는 숫자의 개수가 2^32개니까 `2^32 × 2^12 = 2^4 × 2^40 = 16TB` 만큼의 `물리 메모리를 표현`할 수 있다.
