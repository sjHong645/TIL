...
else if( case 2-1에 해당한다면 )
- p에 대해서 LR회전을 하고 2-2를 이용한다.

else if( case 2-2에 해당한다면 )
- g에 대해서 LL회전하고 p와 g의 색깔을 바꾼다.

else if( case 2-3에 해당한다면 )
- p에 대해서 RL회전하고 2-4를 이용한다.

else if( case 2-4에 해당한다면 )
...
임의의 노드 d를 삭제한다고 하자. 
이진 탐색 트리의 삭제과정은 해당 노드 d를 곧장 없애버리는 게 아니라 d를 대체할 다른 노드 m을 찾아야 한다.

그렇게 d를 대체할 m을 찾고 m에 있는 내용을 기존의 d가 있던 자리에 넣는다.  
그러고 나서 m을 삭제하는 과정까지가 이진 탐색 트리의 삭제과정이다.

이때, m을 삭제하고 나서 RBT의 특성을 만족시킨다면 상관없지만 만족시키지 못할 경우가 있을 수 있다.  
이에 대해서 설명하고자 한다.

그렇다면 m 노드는 자식 노드의 경우에 따라 3가지 경우로 나눌 수 있다.
1) 자식 노드가 없을 때
2) 자식 노드가 하나 일 때
3) 자식 노드가 두 개일 때
⇒  여기서 m 노드는 자식 노드가 2개일 수 없다.  
    왜냐하면, m 노드는 삭제한 노드였던 d 노드의 오른쪽 서브 트리의 최솟값을 가지는 노드이기 때문이다.  
    이진 트리의 특성상 하나의 노드를 기준으로 왼쪽 서브 트리로 이동할 수록 값이 작아지는데  
    m 노드는 트리의 최솟값이므로 더 이상의 왼쪽 서브 트리는 없다.  
    즉, m 노드의 자식 노드는 1개 아니면 0개가 된다. 그래서 이에 대해서만 살펴볼 것이다.


이제부터 위에서 얘기했던 m 노드를 d 노드라 하겠다. 헷갈리지 않길 바랍니다.

d 노드(m 노드)를 삭제하기 이전에 이미 RBT였기 때문에 이에 대한 특성은 모두 만족시킨 상태였을 것이다.

#### d 노드를 가지로 만들 수 있는 3가지 경우
