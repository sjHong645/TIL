...
SSL 클라이언트 인증은 대부분 폼 베이스 인증과 합쳐서 2-factor 인증의 하나로써 사용된다.

2-factor 인증이란 예를 들면, 패스워드라는 1개의 요소뿐만 아니라 이용자가 가진 다른 정보를 병행해서 인증하는 방법이다.

즉, 첫 번재 인증 정보로 SSL 클라이언트 인증을 사용해서 클라이언트의 컴퓨터를 인증하고  
다른 인증 정보로 패스워드를 사용해서 유저의 본인 확인을 하는 방식이다.

### SSL 클라이언트 인증을 사용할 때 비용이 발생함

SSL 클라이언트 인증에는 클라이언트 증명서가 필요한데 이를 사용하기 위해서는 비용이 필요하다.

인증 기관마다 다르지만 증명서 하나 당 연간 수 만원에서 수백만원 정도 한다. 서버 운영자가 스스로 인증 기관을 만들수도 있겠지만 안전하게 운영하기 위해서는 상당한 비용이 발생한다.

## 8.5 폼 베이스 인증 

클라이언트가 서버 상의 웹 어플리케이션에 자격 정보(Credential)를 송신하여 자격 정보의 검증 결과에 따라 인증하는 방식  
(HTTP 사양에 정의된 방식은 아님)

대부분 사전에 등록해둔 자격 정보인 유저 ID와 패스워드를 입력해서 이 내용을 웹 어플리케이션 측에 송신하고 검증 결과를 토대로 검증 성공 여부를 결정한다.

ex. 폼 베이스 인증 예시 - 구글 

![image](https://user-images.githubusercontent.com/64796257/148171570-70e33041-8de8-45d7-9493-09809610c7bd.png)

### 대부분의 인증은 폼 베이스 인증

BASIC, DIGEST 인증은 사용상의 문제와 보안 문제가 많아 거의 사용되지 않고  
SSL 클라이언트 인증은 비용이 많이 들어서 널리 사용되지는 않는다.

웹 사이트의 인증 기능으로 요구하는 기능의 수준을 충족시킨 표준적인 것은 존재하지 않아서 웹 어플리케이션에서 제각각 구현하는 폼 베이스 인증을 사용할 수 밖에 없다.

그러다 보니 웹 사이트 폼 베이스 인증은 별로 다르게 구현된다.  
안전한 방법으로 구현하면 보안 등급이 높지만 그렇지 않을 수도 있다는 것이다.

### 세션 관리와 쿠키에 의한 구현

폼 베이스 인증에서 자주 사용하는 방법으로는 세션 관리를 위해서 쿠키를 사용하는 방법이다.
 
폼 베이스 인증은 서버 쪽의 웹 어플리케이션 등에 의해서 클라이언트가 송신해온 유저 ID와 패스워드가 사전에 등록된 것과 일치하는지를 검증하면서 이뤄진다.

그러나 HTTP는 stateless protocol이라서 방금 전에 인증을 성공했던 유저의 상태를 프로토콜 수준에서는 유지할 수 없다.  

즉, 상태 관리가 안되기 때문에 이후에 똑같은 유저가 접속했더라도 다른 유저로 구분해버린다.

그래서 세션 관리와 쿠키를 이용해서 HTTP에 없는 상태 관리 기능을 보완한다.

![image](https://user-images.githubusercontent.com/64796257/148172489-2d3a78ef-d837-410d-9ac5-d2e6c2c313c6.png)

① 클라이언트가 서버에 유저 ID와 패스워드와 같은 자격 정보를 포함한 request를 송신한다.  
보통은 POST 메소드가 사용되어서 엔티티 바디에 자격 정보가 저장된다. 폼 화면 표시는 HTML / 입력 데이터 송신은 HTTPS 통신을 사용한다.

② 서버 쪽에서는 유저를 식별하기 위해 세션 ID를 발행한다.  
클라이언트로 부터 수신한 자격 정보를 검증하는 것으로 인증을 하고 그 유저의 인증 상태를 세션 ID와 연관지어 서버에서 기록한다.

클라이언트 쪽에서 송신할 때는 Set-Cookie 헤더 필드에 세션 ID를 저장해서 response를 반환한다.  
(세션 ID : 다른 유저와 구별하기 위한 번호 - 때문에 추측하기 어려운 문자열을 사용하고 유효 기간을 관리하는 등 보안에 신경써야 한다)

또한 크로스 사이트 스크립팅 등에 약하다면 피해를 줄이기 위해 httponly 속성도 부여해준다.

③ 서버 쪽에서 세션 ID를 받은 클라이언트는 쿠키로 그 ID를 저장한다.  
이후에 서버에 request를 보낼 때 브라우저가 자동으로 쿠키를 송출하면서 쿠키에 저장했던 세션 ID를 사용한다.

지금까지 소개한 방법은 많은 방법 중 하나에 불과하다.

### 추가 내용

- 폼 베이스 인증은 자격 정보를 교환하는 방법이 표준화가 안되어 있다.
- 패스워드 등의 자격 정보를 서버 측에 어떻게 보존해야하는 지에 대한 내용이 표준화가 안되어 있다. 
